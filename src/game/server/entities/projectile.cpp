...
/*	vec2 n;
	...
	vec2 v = direction;
	t *= speed;
	n.x = p.x + (v.x+config.sv_gravx/100.0)*t;
	n.y = p.y + (v.y+config.sv_gravy/100.0)*t;
	return n;
*/
}
...
void PROJECTILE::tick()
{
	...
	if(targetchr || collide || lifespan < 0)
	{
		...
		game.world.destroy_entity(this);
		if(ownerchar) {
//			game.create_explosion(ownerchar->pos, owner, weapon, false);

			//ownerchar->pos.x = curpos.x;
			//ownerchar->pos.y = curpos.y;
		}
	}
}
