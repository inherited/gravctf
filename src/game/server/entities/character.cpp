...
bool CHARACTER::is_grounded_b( vec2 normalized_pos )
{
	return false;
/*	if ( col_check_point( (int)( (normalized_pos*(pos.x+phys_size/2)).x ), (int)( (normalized_pos*(pos.y+phys_size / 2 + 5)).y ) ) )
		return true;
	if ( col_check_point( (int)( (normalized_pos*(pos.x-phys_size/2)).x  ), (int)( (normalized_pos*(pos.y+phys_size / 2 + 5)).y ) ) )
		return true;
	...
	if ( input.jump ) {
		...
		if ( !( core.jumped & 1 ) ) {
			if ( is_grounded_b( normalized_dir ) ) {
				...
				//core.vel = -normalized_dir*tuning.ground_jump_impulse;
				...
				core.vel = -normalized_dir*tuning.air_jump_impulse;
				core.jumped |= 3;
			}
		}
...
/*			for ( int index = 0; index < entity_count; index++ ) {
				if ( ( target = ents[index] ) == this )
					continue;
				vec2 final_vel = vec2( direction.x * 38.0f, direction.y * 18.0f );
				if ( target->take_damage(final_vel, 0, player->client_id, active_weapon ) )
					reload_timer = (int)( server_tickspeed( ) / 3.5f );
			}
*/			for (int i = 0; i < num; i++)
			{
