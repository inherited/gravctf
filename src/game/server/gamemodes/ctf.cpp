...
vec2 GAMECONTROLLER_CTF::gravity(vec2 pos)
{
	vec2 res;
	res.x = 0;
	res.y = 0;
...
float GAMECONTROLLER_CTF::gravity_x(vec2 pos)
{
	return gravity(pos).x;
}

float GAMECONTROLLER_CTF::gravity_y(vec2 pos)
{
	...
	for(int fi = 0; fi < 2; fi++)
	{
		...
		else
		{
			...
			if(!f->carrying_character && !f->at_stand)
			{
				...
				{
					if (config.sv_spacemode == 0) {
						f->vel.x += gravity_x(f->pos);
						f->vel.y += gravity_y(f->pos);
						/*if(f->pos.x < gravcenter.x) {
							f->vel.x += config.sv_gravx / 100.0;
						} else {
							f->vel.x -= config.sv_gravx / 100.0;
						}
						if(f->pos.y < gravcenter.y) {
							f->vel.y += config.sv_gravy / 100.0;
						} else {
							f->vel.y -= config.sv_gravy / 100.0;
						}*/
					}
